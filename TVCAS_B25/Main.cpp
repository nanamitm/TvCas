...
#include "..\TVCAS_B25\TVCAS.h"
#include "..\TVCAS_B25\TsPacketParser.h"
#include "..\TVCAS_B25\TsDescrambler.h"
	...
	, m_pCardReader(pCardReader)
{
	...
	IniConfig::g_Config.WriteWPrintf(L"Create CCasDevice::CCasDevice: Name=%s\n", m_Name);
}
...
{
	IniConfig::g_Config.WriteWPrintf(L"Call CCasDevice::GetCardCount: %d\n", m_pCardReader->NumReaders());
...
bool CCasDevice::GetCardName(int Index, LPWSTR pszName, int MaxName) const
{
	...
	if (pszName != NULL && MaxName > 0)
		::StringCchCopyW(pszName, MaxName, pszReader);

	IniConfig::g_Config.WriteWPrintf(L"Call GetCardName : ret = %s\n", pszName);
...
#endif
#ifdef CARDREADER_SCARD_DYNAMIC_SUPPORT
	{CCardReader::READER_SCARD_DYNAMIC,	{DEVICE_ID_DYNAMIC,			0,	TEXT("SmartCard Hub"),	TEXT("SmartCard Hub")}},
#endif
};
...
bool CCasManagerB25::GetCasDeviceInfo(int Device, TVCAS::CasDeviceInfo *pInfo) const
{
	...
	IniConfig::g_Config.WriteWPrintf(L"Call GetCasDeviceInfo: %d\n", Device);
...
TVCAS::ICasDevice * CCasManagerB25::OpenCasDevice(int Device)
{
	...
	IniConfig::g_Config.WriteWPrintf(L"Call Open CasDevice: %d\n", Device);
...
bool CCasManagerB25::IsCasDeviceAvailable(int Device)
{
	...
	IniConfig::g_Config.WriteWPrintf(L"Call IsCasDeviceAvailable: %d\n", Device);
...
bool CCasManagerB25::CheckCasDeviceAvailability(int Device, bool *pAvailable, LPWSTR pszMessage, int MaxLength)
{
	...
	IniConfig::g_Config.WriteWPrintf(L"Call CheckCasDeviceAvailability: %d : %s\n", Device, pszMessage);
...
bool CCasManagerB25::OpenCasCard(int Device, LPCWSTR pszName)
{
	...
	IniConfig::g_Config.WriteWPrintf(L"Call Open Cascard: %s type(%d)\n", pszName, m_CasDeviceList[Device].Type);
...
{
	IniConfig::g_Config.WriteWPrintf(L"Call GetCasDevice(): %d\n", m_TsDescrambler.GetCardReaderType());
...
int CCasManagerB25::GetCasCardName(LPWSTR pszName, int MaxName) const
{
	...
	IniConfig::g_Config.WriteWPrintf(L"Call GetCasCardName: %s\n", pszName);
	
	if (pszReader == NULL)
		return 0;

	if (pszName != NULL && MaxName > 0)
		::StringCchCopy(pszName, MaxName, pszReader);
...
int CCasManagerB25::GetInstructionName(int Instruction, LPWSTR pszName, int MaxName) const
{
	...
	if (pszName != NULL && MaxName > 0)
		::StringCchCopy(pszName, MaxName, pszInstruction);
...
{
	CTsDescrambler::InstructionType Type = (CTsDescrambler::InstructionType)Instruction;
